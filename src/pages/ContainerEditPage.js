...
function ContainerEditPage() {
    ...
    const TreeItem = ({ item }) => {
            ...
            return (
                <div>
                    <div style={{ display: 'flex', alignItems: 'center' }}>
                        ...
                        />
                        <span style={{ cursor: 'pointer'}} onClick={() => onNameClick(item)}>
                            {item.name}
                        </span>
                    </div>
                    {!isCollapsed && (
        ...
        return (
            <div
            style={{ display: 'flex', alignItems: 'center', cursor: 'pointer', paddingTop: '8px', paddingLeft: '10px' }}
    ...
    function Tabs({ children }) {
        ...
        return (
            <div className="tabs">
                ...
                {tabs.length > 0 && <div className="tab-panel">{tabs[activeTab].props.children}</div>}
            </div>
        );
    }
    ...
    const fetchFileData = (containerId, filePath) => {
        axios.get(`/api/container/fileContent?containerId={containerId}&filePath={filePath}`, {
            headers: { Authorization: token },
        })
        .then(response => {
