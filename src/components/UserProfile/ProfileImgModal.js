...
function ProfileImgModal({ isOpen, close, profileimgurl }) {
  ...
  const changeUserImgApi = async (e) => {
    ...
    try {
      const response = await axios.patch(baseurl + "api/user/changeProfileImage", formData, {
      ...
      <div className="modal-backdrop">
        <div className="modal">
          ...
          <div className="profile-image-placeholder">
            {(profileImg || previewUserImg) && 
              <img 
                className="userimg"
                alt="userimg" 
                src={previewUserImg ? previewUserImg : profileImg + "?cache=" + Math.random()}
              />
            }
          </div>
          ...
          <div className="buttons">
            <button type="submit" disabled={!file}>프로필 사진 변경</button>
